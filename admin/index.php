...
/*
	...
	if($_POST["username"] && $_POST["password"]){
		$_SESSION['verified'] = 'v3rif3ed-adm1n';
		if(!$conn){
			//error messages
		...
		$query = "SELECT password, clearance FROM users WHERE username = '".$_POST["username"]."';";
		...
		if((int)$clearance < 1){
			$incorrect = true;
			echo "<h1>You do not have clearance to login.</h1>";
			exit(0);
		}
		...
		if(sha1($_POST["password"]) == $hashedPassword){
			...
			$temp = $conn -> query($query)->fetch_assoc()['temp_password'];
			...
			if($temp == '1'){
				...
				header("Location: /inventory/admin/resetPassword.php");
				exit(0);
			}else{
				header("Location: /inventory/admin/adminPanel.php");
				exit(0);
...
<head>
	...
	<title>Inventory - Admin Sign In</title>
...
<body>
	<div id="title" class="contain">
		<span class="textcenter" id="titleText">
			<h1><strong>Inventory</strong> - Admin Sign In</h1>
		</span>
	</div>
	<div class="contain">
